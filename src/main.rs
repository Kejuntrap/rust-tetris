...
use std::fmt;
use block::{TetrisBlocks,BLOCKS,BlockShape,Rotate};
...
impl fmt::Debug for TetrisBoard{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f,"Score:{}. #of block placed:{}. Rotate_angle: {}", self.score,self.block_placed,self.block_rotate)
    }
}
...
impl TetrisBoard{
    ...
    fn init_board() -> Field{
        ...
        for i in 0..BOARDSIZE{
            ...
            if i%TETRIS_WIDTH == EDGE_WIDTH -1  || i%TETRIS_WIDTH == TETRIS_WIDTH-EDGE_WIDTH || i/TETRIS_WIDTH == TETRIS_HEIGHT-EDGE_WIDTH {
                _b[i] = 1;
            }
            else{
                _b[i] = 0;
    ...
    pub fn debug_draw(field: &TetrisBoard ,pos: &Position, block: TetrisBlocks) {
        ...
        for i in 0..=TETRIS_HEIGHT-EDGE_WIDTH{ //y
    ...
    fn is_collision(&self, pos: &Position, block: TetrisBlocks) -> bool{        // 当たり判定
        let _b : BlockShape = BLOCKS[block as usize].rotate(self.block_rotate);
        for y in 0..4{
            for x in 0..4{
                if y+pos.y < TETRIS_HEIGHT && x+pos.x < TETRIS_WIDTH{
                    if self.tetris_board[(y+pos.y)*TETRIS_WIDTH+x+pos.x] != 0 && _b[y][x] != 0 {
                        return true;
                    }
                }
            }
...
fn main() {
    ...
    TetrisBoard::debug_draw(&tet.lock().unwrap(), &pos.lock().unwrap(),*block.lock().unwrap());  //draw
                ...
                if !TetrisBoard::is_collision(&tet, &new_pos, *block) {
                    *pos = new_pos;     // free fall and position update
                    ...
                    for y in 0..4 {
                        for x in 0..4 {
                            if _b[y][x] != 0 {
                                tet.tetris_board[(y+pos.y)*TETRIS_WIDTH+x+pos.x] = _b[y][x];   // block を固定
                            }
                        }
                    }
                        ...
                        if is_erase_line {
                            ...
                            for y2 in (0..y).rev() {
                                for x2 in 0..TETRIS_WIDTH {
                                    tet.tetris_board[(y2+1)*TETRIS_WIDTH+x2] = tet.tetris_board[(y2)*TETRIS_WIDTH+x2];
                                }
                            }
                        }
                    }
        ...
        match g.getch(){        //キー入力
            ...
            Ok(Key::Up) => {
                ...
                loop{
                    ...
                    if TetrisBoard::is_collision(&tet, &Position{x:pos.x,y:new_pos.y+1}, *block){
                        break;
                    }
                    _tmp += 1;
                }
